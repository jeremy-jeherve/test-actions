...
async function checkTestReminderComment( github, context, core ) {
	...
	const touchesDeFusionedFiles = await isTouchingDeFusionedFiles( github, owner, repoName, issue.number, core );

	core.debug( `Build: This PR ${ touchesDeFusionedFiles ? 'touches' : 'does not touch' } files that have been de-fusioned and now need to be tested on WordPress.com.` );
	...
	if (!touchesDeFusionedFiles) {
		...
		if (testCommentIDs.length > 0) {
			core.debug( `Build: this PR previously touched Jetpack de-fusioned files, but does not anymore. Deleting previous test reminder comments.` );
