...
async function getCheckComments( github, owner, repo, number, testCommentIndicator, core ) {
	...
	if ( cache[ cacheKey ] ) {
		core.debug( `get-comments: Returning list of all test reminder comments on ${ cacheKey } from cache.` );
	...
	core.debug( `get-comments: Cached list of all test reminder comments on ${ cacheKey }: ${ testCommentIDs.join(', ') }.` );
